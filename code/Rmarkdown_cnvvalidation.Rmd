...
loadData <- function(patient) {
  ...
  ####### Load PCAWG OTP VCF ##########
  ##################################
  pcawg_otp <- data.frame(read.table(file = paste0("../data/pcawg_cnvs/", patient,"/", patient, "_dkfz.table", sep=""), sep = "\t", header =TRUE))
  pcawg_otp <- pcawg_otp[ , c("CHROM", "POS", "END", "ALT", "TUMOR.TCN")] %>%
  ...
  pcawg_otp_granges <- toGRanges(pcawg_otp)
  ...
  cnv_calls = list(controlfreec = controlfreec, ascat = ascat, cnvkit=cnvkit, pcawg_otp = pcawg_otp_granges, pcawg_svcp = pcawg_svcp_granges)
...
plotData <- function(cnv_calls, patient){ 
  ...
  plotCopyNumberCalls(kp,cn.calls=cnv_calls["ascat"], cn.column = "cn_col", cn.colors = cn.cols, loh.color = loh.col, r0=0.8, r1=0.9, labels="")
  kpAddLabels(kp, labels="ASCAT (sarek)", r0=0.8, r1=0.9, cex=label_size)
  
  plotCopyNumberCalls(kp, cn.calls = cnv_calls["cnvkit"],  cn.colors = cn.cols, loh.color = loh.col, r0=0.6, r1=0.7, labels = "")
  kpAddLabels(kp, labels="CNVKit (sarek)", r0=0.6, r1=0.7, cex=label_size)
  
  plotCopyNumberCalls(kp,cn.calls=cnv_calls["controlfreec"], cn.colors = cn.cols, loh.color = loh.col, r0=0.4, r1=0.5, cn.column = "cn_col", labels="")
  kpAddLabels(kp, labels="ControlFREEC (sarek)",r0=0.4, r1=0.5,  cex=label_size)

  plotCopyNumberCalls(kp, cn.calls = cnv_calls["pcawg_otp"],  cn.colors = cn.cols, loh.color = loh.col,cn.column = "TUMOR.TCN",  r0=0.2, r1=0.3, labels="")
  kpAddLabels(kp, labels="OTP (PCAWG)", r0=0.2, r1=0.3, cex=label_size)
  
  plotCopyNumberCalls(kp, cn.calls = cnv_calls["pcawg_svcp"],  cn.colors = cn.cols, loh.color = loh.col,cn.column = "TUMOUR.TCN", r0=0, r1=0.1, labels="")
  kpAddLabels(kp, labels="SVCP (PCAWG)", r0=0, r1=0.1, cex=label_size)
...
png(paste0(results_folder, "/", "legend_cnvs.png", sep=""), res = res, width = width, height = height, units = units)

# Create an empty plot
plot(1, 1, type="n", ann=FALSE, axes=FALSE)

# Add the legend
legend("bottom", legend=names(cn.cols), fill=cn.cols,  border =  cn.cols, bty="n", cex=0.8, title="CN", horiz = TRUE)
dev.off()
...
get_combination_matrix <- function(df, mode = 'distinct'){
  ...
  otp_gain = df[["pcawg_otp"]][ df[["pcawg_otp"]]$ALT == '<AMP>' ]
  ...
  otp_del = df[["pcawg_otp"]][ df[["pcawg_otp"]]$ALT == '<DEL>' ]
...
plot_upset <- function(sample_comb, title){
  ...
  draw(UpSet(sample_comb[["gain"]], top_annotation = top_ha,
              set_order =c("ascat", "cnvkit", "controlfreec", "svcp", "otp"),
...
# Order is important
cnv_tools <- c("ascat", "cnvkit", "controlfreec","otp", "svcp")
...
mutate(gain_or_del = case_when(
  grepl("gain", type) ~ "Amplification",
  grepl("del", type) ~ "Deletion",
  TRUE ~ type
...
df_combined <- rbind(
  ...
  mutate(otp_plot[["df_long"]], tool = "otp"),
