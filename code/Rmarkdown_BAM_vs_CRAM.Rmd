...
+--------------------------------------+----------------+----------------+-----------+-----------------------------------------------------------------------------------------------------+----------------+-----------+----------------------------------------------------------------------------------------------------+
| Pre-Processing (Spark)               | 1              | yes            | yes       |                                                                                                     | yes            | yes       |                                                                                                    |
+--------------------------------------+----------------+----------------+-----------+-----------------------------------------------------------------------------------------------------+----------------+-----------+----------------------------------------------------------------------------------------------------+
| Pre-Processing (Spark)               | 2              | yes            | yes       |                                                                                                     | yes            | yes       |                                                                                                    |
+--------------------------------------+----------------+----------------+-----------+-----------------------------------------------------------------------------------------------------+----------------+-----------+----------------------------------------------------------------------------------------------------+
| Pre-Processing (Spark)               | 3              | yes            | yes       |                                                                                                     | yes            | yes       |                                                                                                    |
+--------------------------------------+----------------+----------------+-----------+-----------------------------------------------------------------------------------------------------+----------------+-----------+----------------------------------------------------------------------------------------------------+
| Variant calling (germline & somatic) | 1              | yes            | yes       | <https://cfgateway1.zdv.uni-tuebingen.de/orgs/QBiC/workspaces/cfc/watch/2HREZ7WsVnjDE3>\            | yes            | yes       | <https://cfgateway1.zdv.uni-tuebingen.de/orgs/QBiC/workspaces/cfc/watch/Tug8hE0p6xf9t> successful  |
|                                      |                |                |           | successful                                                                                          |                |           |                                                                                                    |
+--------------------------------------+----------------+----------------+-----------+-----------------------------------------------------------------------------------------------------+----------------+-----------+----------------------------------------------------------------------------------------------------+
| Variant calling (germline & somatic) | 2              | yes            | yes       | <https://cfgateway1.zdv.uni-tuebingen.de/orgs/QBiC/workspaces/cfc/watch/1ZMUocwVOySuN> successful   | yes            | yes       | <https://cfgateway1.zdv.uni-tuebingen.de/orgs/QBiC/workspaces/cfc/watch/56Ce7nXg3bPzst> successful |
+--------------------------------------+----------------+----------------+-----------+-----------------------------------------------------------------------------------------------------+----------------+-----------+----------------------------------------------------------------------------------------------------+
| Variant calling (germline & somatic) | 3              | yes            | yes       | <https://cfgateway1.zdv.uni-tuebingen.de/orgs/QBiC/workspaces/cfc/watch/2dbkb24Cz5szug> successful\ | yes            | yes       | <https://cfgateway1.zdv.uni-tuebingen.de/orgs/QBiC/workspaces/cfc/watch/2WqVlGA0faAzd1> successful |
+--------------------------------------+----------------+----------------+-----------+-----------------------------------------------------------------------------------------------------+----------------+-----------+----------------------------------------------------------------------------------------------------+

```{r load_input, echo=FALSE}

cram_directiory = "/Users/monarchy/Projects/sarek_paper/QSARK/data/bam_vs_cram/cram/"
...
cram_spark_preprocessing_1 <- load_data_bam_vs_cram(cram_directiory, "cram_sparkpreprocessing_1", "cram_1", "cram") %>%
  filter(grepl("GATK4_ESTIMATELIBRARYCOMPLEXITY",process) | grepl("GATK4_MARKDUPLICATES_SPARK",process)) 
cram_spark_preprocessing_2 <- load_data_bam_vs_cram(cram_directiory, "cram_sparkpreprocessing_2", "cram_2", "cram") %>%
  filter(grepl("GATK4_ESTIMATELIBRARYCOMPLEXITY",process) | grepl("GATK4_MARKDUPLICATES_SPARK",process)) 
cram_spark_preprocessing_3 <- load_data_bam_vs_cram(cram_directiory, "cram_sparkpreprocessing_3", "cram_3", "cram") %>%
  filter(grepl("GATK4_ESTIMATELIBRARYCOMPLEXITY",process) | grepl("GATK4_MARKDUPLICATES_SPARK",process)) 
...
bam_directiory = "/Users/monarchy/Projects/sarek_paper/QSARK/data/bam_vs_cram/bam/"
...
bam_spark_preprocessing_1 <- load_data_bam_vs_cram(bam_directiory, "bam_sparkpreprocessing_1", "bam_1", "bam") %>%
  filter(grepl("GATK4_ESTIMATELIBRARYCOMPLEXITY",process) | grepl("GATK4_MARKDUPLICATES_SPARK",process)) 
bam_spark_preprocessing_2 <- load_data_bam_vs_cram(bam_directiory, "bam_sparkpreprocessing_2", "bam_2", "bam") %>%
  filter(grepl("GATK4_ESTIMATELIBRARYCOMPLEXITY",process) | grepl("GATK4_MARKDUPLICATES_SPARK",process)) 
bam_spark_preprocessing_3 <- load_data_bam_vs_cram(bam_directiory, "bam_sparkpreprocessing_3", "bam_3", "bam") %>%
  filter(grepl("GATK4_ESTIMATELIBRARYCOMPLEXITY",process) | grepl("GATK4_MARKDUPLICATES_SPARK",process)) 
...
merged_preprocessing_default <- rbind(cram_preprocessing_1, 
                                      ...
                                      cram_spark_preprocessing_1,
                                      cram_spark_preprocessing_2,
                                      cram_spark_preprocessing_3,
                                      bam_preprocessing_1,
                                      bam_preprocessing_2,
                                      bam_preprocessing_3,
                                      bam_spark_preprocessing_1,
                                      bam_spark_preprocessing_2,
                                      bam_spark_preprocessing_3
                                      )
...
test <- format_bam_vs_cram(cram_spark_preprocessing_1)
```

```{r format_input, echo=FALSE}
merged_preprocessing_formatted_all <- format_bam_vs_cram(merged_preprocessing_default)

merged_preprocessing_formatted_all$simple_name <- factor(merged_preprocessing_formatted_all$simple_name, levels = c("MARKDUPLICATES", "MARKDUPLICATES_SPARK", "INDEX_MARKDUPLICATES", "BASERECALIBRATOR", "GATHERBQSRREPORTS", "APPLYBQSR", "MERGE_RECALIBRATED", "INDEX_RECALIBRATED", "ESTIMATELIBRARYCOMPLEXITY", "MOSDEPTH", "SAMTOOLS_STATS"))

merged_variantcalling_formatted_all <- format_bam_vs_cram(merged_variantcalling) 
merged_variantcalling_formatted_all$simple_name <- factor(merged_variantcalling_formatted_all$simple_name, levels = c("DEEPVARIANT", "FREEBAYES", "HAPLOTYPECALLER", "MUTECT2_PAIRED", "GETPILEUPSUMMARIES", "STRELKA", "MANTA", "TIDDIT_SV", "ASCAT", "CNVKIT_BATCH",  "SAMTOOLS_MPILEUP", "MSISENSORPRO_MSI_SOMATIC"))
...
### Peak RSS {.tabset .tabset-fade .tabset-pills}

#### Preprocessing

```{r peakrss_pre, echo=FALSE, results='hide', fig.keep='all'}
peak_rss_preprocessing <- plot_bam_vs_cram_boxplot(df=merged_preprocessing_formatted_all, 
                         xaxis="simple_name", 
                         yaxis="peak_rss_GB", 
                         boxplot_color="format",
                         outputname="/peak_rss_preprocessing", 
                         results_folder=results_folder,
                         yaxisname = "Peak RSS (GB)",
                         reference = "memory_GB")
peak_rss_preprocessing
```

#### Variant calling

```{r peakrss_vc, echo=FALSE, results='hide', fig.keep='all'}
peak_rss_variantcalling <- plot_bam_vs_cram_boxplot(df=merged_variantcalling_formatted_all, 
                         xaxis="simple_name", 
                         yaxis="peak_rss_GB", 
                         boxplot_color="format",
                         outputname="/peak_rss_variantcalling", 
                         results_folder=results_folder,
                         yaxisname = "Peak RSS (GB)",
                         reference = "memory_GB")
peak_rss_variantcalling
```
...
## Combine compute resource plots {.tabset .tabset-fade .tabset-pills}
...
summary_plot <- plot_summary(merged_preprocessing_formatted_compute, 
                             file_name ="/summary_preprocessing", 
                            results_folder=results_folder, legend ="none")
summary_plot
```

### Preprocessing-reduced (Paper)

```{r, echo=FALSE, results='hide', fig.keep='all'}
merged_preprocessing_formatted_compute_reduced <- merged_preprocessing_formatted_compute %>%
  filter(simple_name != 'GATHERBQSRREPORTS') %>%
  filter(simple_name != 'BASERECALIBRATOR') %>%
  filter(simple_name != 'ESTIMATELIBRARYCOMPLEXITY') %>%
  filter(simple_name != 'MOSDEPTH') %>%
  filter(simple_name != 'SAMTOOLS_STATS') %>%
  filter(simple_name != 'INDEX_MARKDUPLICATES') %>%
  filter(simple_name != 'INDEX_RECALIBRATED')

summary_plot_reduced <-  plot_summary(merged_preprocessing_formatted_compute_reduced, 
                             file_name ="/summary_preprocessing_reduced", 
                            results_folder=results_folder, legend="none", angle = 10)
summary_plot_reduced
```
...
summary_variantcalling <- plot_summary(merged_variantcalling_formatted_compute,
                                       file_name ="/summary_variantcalling",
                                        results_folder=results_folder, legend ="none", angle= 30)
...
```{r, echo=FALSE}
merged_preprocessing_sum_storage <- sum_storage(merged_preprocessing_formatted_all)
merged_variantcalling_sum_storage <- sum_storage(merged_variantcalling_formatted_all)
```

### Preprocessing-reduced (Paper)

```{r, echo=FALSE, results='hide', fig.keep='all'}
merged_preprocessing_reduced <- merged_preprocessing_sum_storage %>% 
  filter(simple_name != 'GATHERBQSRREPORTS') %>%
  filter(simple_name != 'BASERECALIBRATOR') %>%
  filter(simple_name != 'ESTIMATELIBRARYCOMPLEXITY') %>%
  filter(simple_name != 'MOSDEPTH') %>%
...
storage_preprocessing_reduced_cumulative_nolog <- plot_cumulative_storage(merged_preprocessing_reduced, "/storage_preprocessing_reduced_cumulative_nolog")
...
```{r, echo=FALSE, results='hide', fig.keep='all'}
storage_preprocessing_cumulative <- plot_cumulative_storage(merged_preprocessing_sum_storage, "/storage_preprocessing_cumulative")
...
```{r, echo=FALSE, results='hide', fig.keep='all'}
storage_variantcalling_cumulative <- plot_cumulative_storage(merged_variantcalling_sum_storage, "/storage_variantcalling_cumulative")
storage_variantcalling_cumulative
```

## Summary: Combine Storage and compute resources {.tabset .tabset-fade .tabset-pills}

### Paper main text

```{r echo=FALSE, results='hide', fig.keep='all'}
prep_paper_plot <- ggpubr::ggarrange(summary_plot_reduced, 
                                     storage_preprocessing_reduced_cumulative_nolog + theme(legend.text = element_text(size = 20)), 
                                     common.legend = TRUE, ncol=2,
                                     labels = c("A","B"), font.label = list(size = 20))

ann_plot <- annotate_figure(prep_paper_plot, top = text_grob("", face = "bold", size = 14))

ggsave(plot=ann_plot, filename = paste0(results_folder,"png/paper_preprocessing", ".png"), 
       device="png", width=40, height=20, units="cm")
ggsave(plot=ann_plot, filename = paste0(results_folder,"eps/paper_preprocessing", ".eps"), 
       device="eps", width=40, height=20, units="cm")
ggsave(plot=ann_plot, filename = paste0(results_folder,"svg/paper_preprocessing", ".svg"), 
       device="svg", width=40, height=20, units="cm")
ann_plot
```
...
```{r echo=FALSE, results='hide', fig.keep='all'}
prep_suppl_plot <- ggpubr::ggarrange(summary_plot,
                                     storage_preprocessing_cumulative,
                                     common.legend=TRUE, ncol=1, align = 'hv', axis="lr", labels = c("A","B"), 
                                     font.label = list(size = 30), vjust=1, hjust=0.1)

prep_suppl_ann_plot <- annotate_figure(prep_suppl_plot, top = text_grob("", face = "bold", size = 14))
 
ggsave(plot=prep_suppl_ann_plot, filename = paste0(results_folder,"png/supplementary_preprocessing", ".png"), 
       device="png", width=50, height = 60, units="cm")
ggsave(plot=prep_suppl_ann_plot, filename = paste0(results_folder,"eps/supplementary_preprocessing", ".eps"), 
       device="eps", width=50, height = 60,  units="cm")
ggsave(plot=prep_suppl_ann_plot, filename = paste0(results_folder,"svg/supplementary_preprocessing", ".svg"), 
       device="svg", width=50, height = 60,  units="cm")
...
```{r echo=FALSE, results='hide', fig.keep='all'}
vc_suppl_plot <- ggpubr::ggarrange(summary_variantcalling,
                                   storage_variantcalling_cumulative,
                                   common.legend=TRUE, ncol=1, align = 'hv', axis = "lr", labels = c("A","B"),  
                                   font.label = list(size = 30), vjust=1, hjust=0.1)
vc_suppl_ann_plot <- annotate_figure(vc_suppl_plot, top = text_grob("", face = "bold", size = 14))
 
ggsave(plot=vc_suppl_ann_plot, filename = paste0(results_folder,"png/supplementary_variantcalling", ".png"), 
       device="png", width=50, height=60, units="cm")
ggsave(plot=vc_suppl_ann_plot, filename = paste0(results_folder,"eps/supplementary_variantcalling", ".eps"), 
       device="eps", width=50, height=60, units="cm")
ggsave(plot=vc_suppl_ann_plot, filename = paste0(results_folder,"svg/supplementary_variantcalling", ".svg"), 
       device="svg", width=50, height=60, units="cm")
vc_suppl_ann_plot
