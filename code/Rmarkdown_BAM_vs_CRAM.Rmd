...
```{r, echo=FALSE, results='hide', fig.keep='all'}
# 
# merged_preprocessing_default_long <- tidyr::gather(merged_preprocessing_default_compute, key="measure", 
#                                                    value="value", c("max_num_cpu","max_peak_vmem_GB", "avg_realtime_min"))
# merged_variantcalling_long <- tidyr::gather(merged_variantcalling_formatted_compute, key="measure", value="value", 
#                                             c("max_num_cpu","max_peak_vmem_GB", "avg_realtime_min"))
...
For each process that support scatter/gather, i.e. APPLYBQSR, the process with the longest runtime is kept, reducing the datapoint per sample & repetition to one. 
For all processes, the three replicates for each data type are combined by taking the avg runtime, the max cpu & memory usage over each group, further reducint the data points per box plot from 30 to 10. 
...
summary_variantcalling <- plot_summary(merged_variantcalling_formatted_compute, 
                                       file_name ="/summary_variantcalling.png", 
                                        results_folder=results_folder)# plot_summary(merged_variantcalling_long, "/summary_variantcalling.png")
